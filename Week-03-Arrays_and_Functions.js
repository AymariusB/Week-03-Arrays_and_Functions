...
myTodoList.push('wash dishes');
myTodoList.push('buy groceries');
...
let yourTodoList = [];
yourTodoList.push('make reservation at the restaurant');
yourTodoList.push("make an appointment with the doctor's office");
console.log(yourTodoList);
// create another array, ourTodoList
let ourTodoList = [];
// combine myTodoList and yourTodoList into ourTodoList
ourTodoList.push(myTodoList);
ourTodoList.push(yourTodoList);
...
function reverse(ourTodoList = [myTodoList, yourTodoList]){
return ourTodoList.reverse();
}
console.log(ourTodoList);
